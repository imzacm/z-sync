...
fn bench_uncontended(c: &mut Criterion) {
    ...
    group.bench_function("z_sync::Lock (Async)", |b| {
        let m = ZLock::new(Payload(0));
...
fn bench_read_only(c: &mut Criterion) {
    ...
    bench_blocking!(group, "z_sync::Lock (Blocking)", ZLock::new(Payload(0)), |_i, m| black_box(
        m.read().0
    ));
    ...
    bench_async!(group, "z_sync::Lock (Async)", &rt, ZLock::new(Payload(0)), |_i, m| black_box(
        m.read_async().await.0
    ));
...
fn bench_read_heavy(c: &mut Criterion) {
    ...
    bench_blocking!(group, "z_sync::Lock (Blocking)", ZLock::new(Payload(0)), |i, m| {
        if i % 10 == 0 {
            black_box(m.write().0 += 1);
        } else {
            black_box(m.read().0);
    ...
    bench_async!(group, "z_sync::Lock (Async)", &rt, ZLock::new(Payload(0)), |i, m| {
        if i % 10 == 0 {
